<?php
    ...
    if (isset($_GET["op"]) && $_GET["op"] === "ajout") {
        ...
        if(!$DAO->verifieurExistenceCategorie((int)$identifiants['idCategorie'])) {
            $errors["error303"] = "Le numéro de catégorie n'existe pas";
        }
        if(!$DAO->verifieurDoublonPseudo($identifiants['login'])) {
        ...
        $identifiants['login'] = (isset($_POST['login'])?$_POST['login']:"");
        $identifiants['mdp'] = (isset($_POST['mdp'])?$_POST['mdp']:"");
        $bool = $DAO->verificationLoginPassword($identifiants["login"], $identifiants["mdp"]);
